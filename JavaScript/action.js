...
function likeTweet(tweet_id) {
    ...
    likeTweetRequest.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            if (this.responseText.includes("tweet")) {
                document.getElementById(tid + 'likes').innerHTML = this.responseText;
                document.getElementById(tid + 'best').innerHTML = this.responseText;
            } else {
                document.getElementById(tid + 'likes').innerHTML = this.responseText;
                document.getElementById(tid + 'best').innerHTML = this.responseText;
                document.getElementById(tid + 'follow').innerHTML = this.responseText;
            }
        }
    };
